...
--------------------------------------------------------
-- Truy vấn 2.1: Cho biết những đơn hàng đang chờ giao ở TP.HCM. Kết quả sắp xếp tăng dần theo thời gian đặt hàng.
GO
--CREATE
ALTER PROC DH_CHO_GIAO 
@TINH NVARCHAR(40)
AS
BEGIN
	SELECT DH.*, NV.TEN_NV, KH.TENKH, V.PHAN_TRAM_GIAM_GIA, V.TG_BAT_DAU, V.TG_KET_THUC
	FROM (SELECT * FROM DON_HANG
	WHERE TRANG_THAI = N'Chờ giao' AND TP_TINH = @TINH) DH
	JOIN NHAN_VIEN NV ON NV.MANV = DH.MANV
	JOIN KHACH_HANG KH ON KH.MAKH = DH.MAKH
	JOIN VOUCHER V ON V.MA_VOUCHER = DH.MA_VOUCHER
	ORDER BY TG_LAP_DH
END
GO
EXEC DH_CHO_GIAO N'Bình Định'
GO
ALTER 
--CREATE 
PROC USP_XEMCTDH  
...
BEGIN
	...
	SELECT DH.*, NV.TEN_NV, KH.TENKH, V.PHAN_TRAM_GIAM_GIA, V.TG_BAT_DAU AS VOUCHER_BD, V.TG_KET_THUC AS VOUCHER_KT 
	FROM (SELECT * FROM DON_HANG
	WHERE MADH = @MADH) DH
	JOIN NHAN_VIEN NV ON NV.MANV = DH.MANV
	JOIN KHACH_HANG KH ON KH.MAKH = DH.MAKH
