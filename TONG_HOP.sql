...
BEGIN
	...
	SET THANH_TIEN_MUA = inserted.SO_LUONG_MUA * ( SAN_PHAM.GIA_BAN- inserted.GIAM_GIA* SAN_PHAM.GIA_BAN )
	...
	SET PHI_SAN_PHAM = (SELECT SUM(CHI_TIET_DON_HANG.THANH_TIEN_MUA) 
						FROM CHI_TIET_DON_HANG JOIN inserted ON CHI_TIET_DON_HANG.MADH = inserted.MADH
...
BEGIN
	...
	IF EXISTS (SELECT DH.MADH FROM DON_HANG DH JOIN inserted ON inserted.MADH = DH.MADH WHERE DH.MA_VOUCHER IS NOT NULL)
		BEGIN
			...
			SET DON_HANG.PHI_GIAM = (SELECT CASE 
									...
									WHEN (DON_HANG.PHI_SAN_PHAM * V.PHAN_TRAM_GIAM_GIA / 100) < V.GIAM_TOI_DA 
										THEN DON_HANG.PHI_SAN_PHAM * V.PHAN_TRAM_GIAM_GIA / 100
									WHEN (DON_HANG.PHI_SAN_PHAM * V.PHAN_TRAM_GIAM_GIA / 100) >= V.GIAM_TOI_DA THEN V.GIAM_TOI_DA
									END
									FROM VOUCHER V
	...
	--Ràng buộc giảm giá < phí vận chuyển + phí sản phẩm thì mới được giảm
	UPDATE DON_HANG
